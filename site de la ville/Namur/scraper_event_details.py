...
def extract_location(ul_details):
    """Extraire le lieu et l'adresse (wrapper Namur)."""
    return extract_location_plone(ul_details, default_city="Namur")


def extract_image(event_div):
    """Extraire l'image (wrapper Namur)."""
    return extract_image_plone(event_div, BASE_DOMAIN)


def extract_category(ul_details):
    """Extraire la catégorie depuis li.categories_evenements (spécifique Namur)."""
    li = get_detail_li(ul_details, "categories_evenements")
    if li:
...
def extract_pricing(ul_details):
    """Extraire le coût depuis li.taxonomy_gratuite (spécifique Namur)."""
    li = get_detail_li(ul_details, "taxonomy_gratuite")
    if li:
        return extract_label_value(li)
    return ""
...
def scrape_event(url):
    """Scraper les détails d'une page événement Namur."""
    try:
...
def main():
    ...
    json_files = [f for f in os.listdir(script_dir) if f.startswith("namur_") and f.endswith(".json")]
    if not json_files:
        logger.error("Aucun fichier namur_*.json trouvé. Lancez d'abord scraper_agenda.py.")
        return
